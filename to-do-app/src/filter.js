...
export const checkWithFilter = (toDoItem) => {
  ...
  const notAnyUrgencyApplied = data.urgencyFilter.every(element => element === 0);
  const notAnyCategoryApplied = data.categoryFilter.every(element => element === 0);
...
export const containSearchedWord = (searchedText, title) => (!searchedText || title.toLowerCase().indexOf(searchedText) + 1);
